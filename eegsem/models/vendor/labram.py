"""Vendored from https://github.com/935963004/LaBraM (MIT License, Wei-Bang Jiang et al., ICLR 2024). timm deps replaced by shims."""
...
class NeuralTransformer(nn.Module):
    ...
    def forward_features(
        self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs
    ):
        ...
        pos_embed_used = (
            self.pos_embed[:, input_chans] if input_chans is not None else self.pos_embed
        )
        if self.pos_embed is not None:
            pos_embed = (
                pos_embed_used[:, 1:, :]
